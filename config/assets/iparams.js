...
const validateFdCredentials = async (domain, api_key, isTrue) => {
  try {
    ...
    console.error(error);
    if (isTrue) showNotification("error", "Invalid API Key.");
    else showNotification("error", "Invalid Domain or API Key.");
    return false;
  }
};
...
const handleVerifyBtn = async (rowId) => {
  ...
  try {
    ...
    } else handlePhillElement("red", rowId);
  } catch (error) {
    console.error(error);
  } finally {
